...
namespace Jrainstar
{
    public abstract class MessageSessionHandler<Message> : IMessageHandler where Message : MessageObject
    {
        protected abstract Task Run(Session session, Message message);

        public void Handle(Session session, object msg)
        {
            HandleAsync(session, msg);
        }

        private async Task HandleAsync(Session session, object message)
        {
    ...
    public abstract class MessageSessionHandler<Request, Response> : IMessageHandler where Request : MessageObject, IRequest where Response : MessageObject, IResponse
    {
        protected abstract Task Run(Session session, Request request, Response response);

        public void Handle(Session session, object message)
        {
            HandleAsync(session, message);
        }
