        ...
        void IProtoSerializer.EmitRead(Compiler.CompilerContext ctx, Compiler.Local valueFrom)
        {
            ...
            using (Compiler.Local fieldNumber = new Compiler.Local(ctx, ctx.MapType(typeof(int))))
            {
                ...
                foreach (BasicList.Group group in BasicList.GetContiguousGroups(fieldNumbers, serializers))
                {
                    ...
                    int groupItemCount = group.ItemMap.Count;
                    if (groupItemCount == 1)
                    {
                        ...
                        WriteFieldHandler(ctx, expected, loc, processThisField, @continue, (IProtoSerializer)group.ItemMap[0]);
                    }
                    else
                    {   // implement as a jump-table-based switch
                        ...
                        for (int i = 0; i < groupItemCount; i++)
                        {
                            WriteFieldHandler(ctx, expected, loc, jmp[i], @continue, (IProtoSerializer)group.ItemMap[i]);
                        }
                    }
